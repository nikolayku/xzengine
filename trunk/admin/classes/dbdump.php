...
//////////////////////////////////////////////////////////////////////////////////////////////
// модуль для работы с созданием резервной копии - востановлением БД mysql
...
////////////////////////////////////////////////////////////
//dumper отвечает за бэкап базы данных
////////////////////////////////////////////////////////////
...
{	
	...
	function dumper()
	{
		...
		$this->tabs = 0;			
		$this->records = 0;			// количество записей в БД
		$this->size = 0;			// размер таблицы
		$this->comp = 0;
	...
	// $method - 0 без сжатия , 1 GZip
	// $level - уровень компрессии
	///////////////////////////////////////////////	
	
	function backup($method, $level)
	{
		
		set_error_handler("SXD_errorHandler");
		$buttons = "<A ID=save HREF='' STYLE='display: none;'>Скачать файл</A> &nbsp; <INPUT ID=close TYPE=button VALUE='Закрыть окно' DISABLED onClick=\"window.close();\">";
		echo tpl_page(tpl_process("Создается резервная копия БД"), $buttons);

		$this->SET['last_action']     = 0;
		$this->SET['last_db_backup']  = DATABASE_NAME;		// имя бд для сохранения
		$this->SET['tables_exclude']  = 1;					// включен ли фильтр таблиц
		$this->SET['tables']          = '';					// сам фильтр таблиц
		$this->SET['comp_method']     = $method;			// метод	
		$this->SET['comp_level']      = $level;				// уровень
		...
		echo tpl_l("Размер БД:       " . ConvertBytes($this->size), C_RESULT);
		$filesize = ConvertBytes(filesize(PATH . $this->filename));
		...
		echo "<SCRIPT>with (document.getElementById('save')) {style.display = ''; innerHTML = 'Скачать файл ({$filesize})'; href = '" . URL . $this->filename . "'; }document.getElementById('close').disabled = 0;</SCRIPT>";
	}
	...
	function restore($fname)
	{
		...
		$this->SET['last_db_restore'] = DATABASE_NAME;	// имя бд куда востанавливать
		$file						  = $fname;
		...
		$this->comp = $this->SET['comp_method'] * 10 + $this->SET['comp_level'];
		echo "<SCRIPT>document.getElementById('close').disabled = 0;</SCRIPT>";
...
<HTML>
<HEAD>
<TITLE>xzengine dumper based on Sypex Dumper Lite sources</TITLE>
...
<B>Резервное копирование / восстановление</B><IMG ID=GS WIDTH=1 HEIGHT=1 STYLE="visibility: hidden;"></TD>
</TR>
<TR>
...
{
	return <<<HTML
	<SCRIPT>document.getElementById('close').disabled = 0;</SCRIPT>
...
//////////////////////////////////////////////////////////
// сообщение о неудачной попытке учтановить соединение с БД
//////////////////////////////////////////////////////////
...
//////////////////////////////////////////
// callback функция обработки ошибок
//////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////
// Создание резервной копии - востановление с резервной копии
///////////////////////////////////////////////////////////////////////////
...
// если используется текстовая БД
if(!DATABASE_USEMYSQL)
{
	$buttons = "<INPUT ID=close TYPE=button VALUE='Закрыть окно' DISABLED onClick=\"window.close();	\">";
	echo tpl_page("Текущяя БД не поддерживает операции создания резервной копии - востановления из резервной копии",$buttons);
	echo tpl_enableBack();
	exit();	
}
...
// устанавливаем функцию - обработчик ошибок
...
{
	$buttons = "<INPUT ID=close TYPE=button VALUE='Закрыть окно' DISABLED onClick=\"window.close();\">";
	echo tpl_page(tpl_error(''),$buttons);
	echo tpl_enableBack();
	exit();
}
...
{	
	switch($_GET['action'])
	{
		case 'backup':
		{	// СОЗДАНИЕ копии БД
			$SK->backup($_POST['CompressionMethod'], $_POST['CompressionLevel']);
			break;
		}
		case 'restore':
		{	// востановление
