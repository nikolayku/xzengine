...
{	
	...
	function SaveConfig($filename = 'test.php')
	{
		...
		fprintf($handle, '<?php'."\n");
		fprintf($handle, '//---------------------------------------------'."\n");
		fprintf($handle, '// файл настроек создан '.date("l dS of F Y h:i:s A")."\n");
		fprintf($handle, '//---------------------------------------------'."\n");
		
		fprintf($handle, "\n".'// Состояние сайта '."\n\n");
		$this->SaveParam($handle, 'OFF_SITE', $_POST['OFF_SITE'], '// Отключить сайт');
		$this->SaveParam($handle, 'OFF_SITE_MESSAGE', $_POST['OFF_SITE_MESSAGE'], '// Сообщение пользователям');
		$this->SaveParam($handle, 'DEBUG_MODE', $_POST['DEBUG_MODE'], '// состояние debug режима');
		
		fprintf($handle, "\n".'// работа с БД'."\n\n");
		
		$this->SaveParam($handle, 'DATABASE_HOST', $_POST['DATABASE_HOST'], '// имя хоста с БД');
		$this->SaveParam($handle, 'DATABASE_USER', $_POST['DATABASE_USER'], '// пользователь');
		$this->SaveParam($handle, 'DATABASE_PASSWORD', $_POST['DATABASE_PASSWORD'], '// пароль');		
		$this->SaveParam($handle, 'DATABASE_NAME', $_POST['DATABASE_NAME'], '// имя базы данных');		
		$this->SaveParam($handle, 'DATABASE_TBLPERFIX', $_POST['DATABASE_TBLPERFIX'], '// перфикс таблиц при создании');		
		$this->SaveParam($handle, 'DATABASE_USEMYSQL',DATABASE_USEMYSQL, '// использование mysql или текстовой бд. устанавливается только при установке, дальнейшее изменение возможно только "руками"');
				
	
		fprintf($handle, "\n".'// настройки языковой поддержки'."\n\n");		
		$this->SaveParam($handle, 'SITE_LOC_FILE', $_POST['SITE_LOC_FILE'], '// путь к файлу локализаций для сайта');
		$this->SaveParam($handle, 'ADMINPANEL_LOC_FILE', $_POST['ADMINPANEL_LOC_FILE'], '// путь к файлу локализаций для админпанели');

		fprintf($handle, "\n".'// поддержка GZip'."\n\n");		
			
		$this->SaveParam($handle, 'GZIP_ENABLED', $_POST['GZIP_ENABLED'], '// поддержка gzip');		
		$this->SaveParam($handle, 'GZIP_COMPRESSION', $_POST['GZIP_COMPRESSION'], '// уровень компрессии');
		
		fprintf($handle, "\n".'// Настройки сайта'."\n\n");		
		
		$this->SaveParam($handle, 'SITE_PATH', $_POST['SITE_PATH'], '// путь к сайту');
		$this->SaveParam($handle, 'NEWSPERPAGE', $_POST['NEWSPERPAGE'], '// количество новостей на странице');
		$this->SaveParam($handle, 'SITE_TITLE', $_POST['SITE_TITLE'], '// заголовок сайта');
		$this->SaveParam($handle, 'SITE_KEYWORDS', $_POST['SITE_KEYWORDS'], '// ключевые слова');
		$this->SaveParam($handle, 'SKIN', $_POST['SKIN'], '// скин для сайта');
		$this->SaveParam($handle, 'DATEFORMAT', $_POST['DATEFORMAT'], '// формат даты');		
		$this->SaveParam($handle, 'SIMPLY_URL', $_POST['SIMPLY_URL'], '// выдавать ссылку в удобном виде или нет требует возможность записи в файл .htaccess');		
		$this->SaveParam($handle, 'USERS_CAN_ADD_NEWS', $_POST['USERS_CAN_ADD_NEWS'], '// Могут ли обычные пользователи добавлять новости');		
					

		fprintf($handle, "\n".'// админпанель'."\n\n");

		$this->SaveParam($handle, 'ADMINPANEL_SKIN', $_POST['ADMINPANEL_SKIN'], '// скин по умолчанию для админпанели');		
		$this->SaveParam($handle, 'ADMINPANEL_NEWSPERPAGE', $_POST['ADMINPANEL_NEWSPERPAGE'], '// количество новостей на странице редактирования');		
		
		fprintf($handle, "\n".'// Rss Настройки'."\n\n");
		
		$this->SaveParam($handle, 'RSS_NEWS', $_POST['RSS_NEWS'], '// сколько будет выдаваться новостей');		
		$this->SaveParam($handle, 'RSS_DESCRIPTION', $_POST['RSS_DESCRIPTION'], '// описание ресурса');		
		$this->SaveParam($handle, 'RSS_LIVETIME', $_POST['RSS_LIVETIME'], '// время через которое следует обновлять содержимое Rss канала');		
		
		fprintf($handle, "\n".'// загрузка файлов'."\n\n");
		$this->SaveParam($handle, 'UPLOADFILE_SIZE', $_POST['UPLOADFILE_SIZE'], '// максимальный размер файла загружаемого на сервер');		
		$this->SaveParam($handle, 'UPLOADFILE_DIRECTORY', $_POST['UPLOADFILE_DIRECTORY'], '// директория куда сохранять загруженные файлы');		
		
		fprintf($handle, "\n".'// путь к форуму'."\n\n");
		$this->SaveParam($handle, 'FORUM_PATH', $_POST['FORUM_PATH'], '// Путь к форуму , если пост сохраняется в бд , то этот текст заменяется на тег {forum_path}');		
